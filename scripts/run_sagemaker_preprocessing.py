"""
SageMaker Processing job to run prepare_data.py in the cloud.
...
def main():
    """Create and run SageMaker Processing job."""
    ...
    sklearn_processor = SKLearnProcessor(
        ...
        role=role_arn,
        instance_type="ml.m5.large",  # Need more memory for large dataset (1.2 GB)
        instance_count=1,
    ...
    print(f"[INFO] Input data: {RAW_DATA_S3}")
    print(f"[INFO] Output location: {PROCESSED_DATA_S3}")
    
    # Define inputs (S3 -> SageMaker container)
    inputs = [
        ProcessingInput(
            source=RAW_DATA_S3,
    ...
    # Define outputs (SageMaker container -> S3)
    outputs = [
        ProcessingOutput(
    ...
    job_name = f"student-ml-prepare-data-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    
    print("\n" + "=" * 60)
    print("Starting SageMaker Processing Job")
    print("=" * 60)
    print(f"Job name: {job_name}")
    print(f"Input: {RAW_DATA_S3} -> {SAGEMAKER_INPUT}")
    print(f"Output: {SAGEMAKER_OUTPUT} -> {PROCESSED_DATA_S3}")
    ...
    try:
        ...
        print("=" * 60)
        print(f"Check outputs in: {PROCESSED_DATA_S3}")
        print("\nExpected files:")
        print("  - train.csv")
        ...
        print(f"4. Verify raw data exists at: {RAW_DATA_S3}")
        sys.exit(1)
